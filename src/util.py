...
def faster(QT, l, T_t, T, i, n, m, QT_first, mean, dev, I, P):
    ...
    _set_exclusion_zone(i, m // 2, len(D), D)
    a = np.isnan(D)
    if np.any(a):
        D[a] = np.inf
    I[P > D] = i
...
def find_aligned_motifes(series, matrix_profile, matrix_index, m, num_motives=16, start=0, offset=1):
    result = []
    exc_zone_len = np.round(2 * m)
    t_mean, t_std = timeseries_mean_stddev(series, m)
    nb = len(series) - m + 1
    mp = matrix_profile.copy()[start::offset]
    mi = matrix_index.copy()[start::offset]
    for _ in range(num_motives):
        motives = []

        idx = np.argmin(mp)
        dist_len = mp[idx]
        dist_len =  2 * dist_len+ 0.05#* dist_len
        print("============", dist_len)
        mp_index = idx * offset + start
        motives.append((series[mp_index : mp_index + m], mp_index))

        dist_profile = calculateDistanceProfile(series[mp_index:mp_index + m], 
                        series, 
                        slidingDotProduct(series[mp_index:mp_index + m], series)
                        ,t_mean,
                        t_std,
                        np.mean(series[mp_index:mp_index + m]), 
                        np.std(series[mp_index:mp_index + m]))
        _set_exclusion_zone(mp_index, exc_zone_len, nb, dist_profile)

        mp[idx] = np.inf

        dp_idx = dist_profile[start::offset].argsort()
        dp = dist_profile[start::offset]
        dp.sort()
        for i, idx in enumerate(dp_idx):
            mp_index = idx * offset + start
            if dp[i] > dist_len:
                break
            if mp[idx] != np.inf:
                motives.append((series[mp_index: mp_index + m],mp_index))
                #_set_exclusion_zone(idx, exc_zone_len, len(mp), mp)
                mp[idx] = np.inf
        print(len(motives))
...
def find_motifes2(series, matrix_profile:np.ndarray, matrix_index:np.ndarray, m, num_motives = 16):
    ...
    for _ in range(num_motives):
        ...
        idx = np.argmin(mp)
        dist_len = mp[idx]
        dist_len =  1.5  *  dist_len * dist_len + 0.1
        print("============", dist_len)
        ...
        _set_exclusion_zone(idx, exc_zone_len, nb, dist_profile)
        _set_exclusion_zone(idx, exc_zone_len, nb, mp)
        if np.any(np.isnan(dist_len)):
            print(len(motives))
            result.append(motives)
            continue
        a = np.isnan(dist_profile)
        if np.any(a):
            dist_profile[a] = np.inf
...
def _set_exclusion_zone(idx, exc_zone_len, nb, mp):
    exc_zone_start = 0 if idx < exc_zone_len // 2 else idx - exc_zone_len // 2
    exc_zone_end = nb if idx > nb - exc_zone_len // 2 else idx + exc_zone_len // 2
